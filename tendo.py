...
#
# -*- coding: utf-8 -*-
#version 3.7 ta3 py w lekhra ta3 pyqt5 w numpy w minimalmodbus w 0.10.0 ta3 pyqtgraph
"""
...
def updatePlot():
    curves[0].setData(data1)#curves[0]=graph 1 normalement
...
def treeViewBuilder():#manich fahemha bien 
    line_count = 0
    index_cnt=0
    global csv_reader
    global paramsPB
...
def updateCurvePlot():
    ...
    try:
        ...
        val=instr.read_registers(27, 7, functioncode=4)#27 rahi fel csv file li hawelto lexel 27 hiya raw_s1 
        updateCurveVals(val[0], val[1], val[2], val[3], val[4], val[5], val[6])
        if plotLen==10:#bah yesra taakhir koul 10 9irayat ydir mise a jour fel graph khatra  
            updatePlot()
            plotLen=1
        plotLen=plotLen+1
...
def updateRI():# hadi te9ra w dir update lel input registers(limate9derch tmodifi fihom/read only)
#    print("updateRI")
    global doDelayRead
    if doDelayRead > 0:
...
def updateRH():# hadi te9ra w dir update lel holding registers(li te9der tmodifi fihom/read and write)
    global doDelayRead
    global doUpdateRH
    try:
        #print("RH-update")
...
def setRH(path, data):
    try:
        if path[0] != "Holding registers":#ida mahouch RH matkhalikch tmodifi 
            return
        for i in paramsPB[2]["children"]:
